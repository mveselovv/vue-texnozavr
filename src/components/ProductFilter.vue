          ...
          <li class="colors__item" v-for="color in colors" :key="color.id">
            <label class="colors__label">
              <input class="colors__radio sr-only" type="radio" name="color" :value="color.id"
                v-model.number="currentNumberColor">
...
export default {
  ...
  props: ['priceFrom', 'priceTo', 'categoryId', 'colorNumber'],
  computed: {
    categories() {
      return this.categoriesData ? this.categoriesData.items : []
    },
    colors() {
      return this.colorsData ? this.colorsData.items.filter(item => item.id !== 12) : [];
    }
  },
  watch: {
    priceFrom(value) {
  ...
  methods: {
    submit() {
      ...
      this.$emit('update:colorNumber', this.currentNumberColor);
    },
    reset() {
      this.$emit('update:priceFrom', 0);
      this.$emit('update:priceTo', 0);
      this.$emit('update:categoryId', 0);
      this.$emit('update:colorNumber', 0);
    },
    loadCategories() {
